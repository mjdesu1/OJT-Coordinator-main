...
class MYPDF extends TCPDF
{
    ...
    {
        // Add the ASSCAT logo (left) and department logo (right)
        $this->Image('https://tse1.mm.bing.net/th?id=OIP.Io0CSZ-c6H2XA9KnczZmLAHaHa&pid=Api&P=0&h=220', 10, 10, 20); // ASSCAT logo on the left
        $this->Image('https://tse4.mm.bing.net/th?id=OIP.oZn4j3_rbFhI7mU7XlRqMwHaHa&pid=Api&P=0&h=220', 180, 10, 20); // Department logo on the right

        // Set font and color for the text
        $this->SetTextColor(0, 0, 0);

        // Move the cursor down a bit to avoid overlap with logos
        $this->Ln(5);
        $this->SetFont('helvetica', 'R', 10);
        $this->Cell(0, 5, 'Republic of the Philippines', 0, 1, 'C', 0);

        $this->SetFont('helvetica', 'B', 10);
        $this->Cell(0, 7, 'AGUSAN DEL SUR STATE COLLEGE OF AGRICULTURE AND TECHNOLOGY', 0, 1, 'C', 0);

        $this->SetFont('helvetica', 'I', 12);
        $this->Cell(0, 5, 'Bunawan, Agusan del Sur', 0, 1, 'C', 0);

        $this->SetFont('helvetica', 'R', 8);
        $this->Cell(0, 4, 'Tel. No: (085) 343-0760 | Email: info@asscat.edu.ph', 0, 1, 'C', 0);

        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(0, 4, 'www.asscat.edu.ph', 0, 1, 'C', 0, 'https://www.asscat.edu.ph/', 0, false, 'T', 'M');
